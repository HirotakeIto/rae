...
AnalysisEnvironmentT23 <- R6::R6Class(
  ...
  public = list(
    ...
    get_fm_basic = function() "{dependent} ~ is_apirl + as.factor(year) | as.factor(lived_city) | 0 | lived_city" %>% strformat(c(dependent = self$target)) %>% as.formula(),
    get_fm = function() {
...
main = function(save_folder_basis) {
  ...
  # analysis setup
  targets = c(
    "age_father", "age_mother", "I(no_legitimate*100)", 
    ...
    for (type_formula in c("basic")) { 
      for (type_sampling in c("in_april_and_march")) {
        ana_env = AnalysisEnvironmentT23$new(
          ...
          tag = c(type_sampling, "t35", type_formula, target)
        )
  ...
  summary_glance = ana_envs %>%
    ...
    purrr::reduce(dplyr::bind_rows)
  # save
  savefolder = file.path(save_folder_basis, "t35")
